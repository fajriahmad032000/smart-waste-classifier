...
import time
import pandas as pd
import random  # 🧠 TAMBAHAN: Diperlukan untuk fungsi generate_dummy_data agar tidak error
import plotly.express as px  # 📊 TAMBAHAN: Diperlukan untuk membuat grafik px.bar agar tidak error
import firebase_admin
from firebase_admin import credentials, firestore

# 👇 SISTEM WAKTU LOKAL INDONESIA
...
# --- TAMPILAN APLIKASI UTAMA (JIKA SUDAH LOGIN BERHASIL) ---
    ...
    try: 
        ...
        model = None
        st.sidebar.error(f"⚠️ Detail Error Model: {e}")
            ...
            if st.button("Mulai Analisis AI"):
                if model is None:
                    ...
                    with st.spinner("🤖 AI sedang menganalisis objek..."):
                        ...
                        # ⚡ UPDATE LOKAL REAL-TIME (SINKRONISASI GRAFIK DAN RIWAYAT INSTAN)
                        data_baru_lokal = {
                            'waktu_klasifikasi': waktu_sekarang,
                            'jenis_sampah': predicted_class,
    ...
    # --- HALAMAN 2: DASHBOARD SMART WASTE ---
        ...
        if st.session_state.history:
            ...
            df['jenis_sampah'] = df['jenis_sampah'].str.lower().str.strip()
            ...
            with col2: st.metric("Kategori Dominan", df_filtered['jenis_sampah'].mode()[0].lower() if not df_filtered.empty else "n/a")
            ...
            with row1_col1:
                ...
                if not df_graph.empty:
                    ...
                    color_map_lokal = {
                        'organik': '#2e7d32', 'plastik': '#fbc02d', 'kertas': '#ff9800',
                        'logam': '#78909c', 'kaca': '#0288d1', 'unorganik': '#d32f2f'
                    }
    ...
    # --- HALAMAN 4: RIWAYAT KLASIFIKASI ---
        ...
        if st.session_state.history:
            for item in st.session_state.history[:30]: 
                st.info(f"⏳ **[{item['waktu_klasifikasi']}]** &nbsp;|&nbsp; Kategori Sampah: **{item['jenis_sampah'].lower()}** &nbsp;|&nbsp; Efisiensi Sistem: **{item['efisiensi_daur_ulang']}**")
        else:
