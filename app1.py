...
def inisialisasi_firebase():
    """Fungsi otomatis: membaca Secrets di internet atau file lokal di laptop"""
    if not firebase_admin._apps:
        # 1. JIKA BERJALAN DI WEB INTERNET (STREAMLIT CLOUD)
        if "firebase" in st.secrets:
            firebase_info = dict(st.secrets["firebase"])
            # Mengatasi kendala enter (\n) yang sering merusak private key di server cloud
            firebase_info["private_key"] = firebase_info["private_key"].replace("\\n", "\n")
            cred = credentials.Certificate(firebase_info)
        
        # 2. JIKA BERJALAN DI LAPTOP LOKAL AHMAD
        else:
            try:
                # Coba cara standar dulu
                cred = credentials.Certificate('kunci-firebase.json')
            except:
                # Jika Windows iseng menambahkan .json ganda di latar belakang
