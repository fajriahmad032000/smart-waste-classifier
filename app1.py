...
import time
# 👇 TAMBAHKAN LINE INI DI BAGIAN UTAMA IMPORT
...
# ===================== 4. UTILITIES (LOGIN & DATA) =====================
def cek_login_firebase(username_input, password_input):
    """Memeriksa kecocokan data login ke koleksi 'pengguna' di Firestore"""
    if db is None:
        return False
    try:
...
def ambil_data_dari_firebase():
    if db is None:
        return []
    try:
        # Mengambil data dari koleksi log_sampah
        docs = db.collection('log_sampah').stream()
        data_list = []
        
        for doc in docs:
            d = doc.to_dict()
            # Ambil nilai dengan pengaman .get() agar jika kolom tidak ada, aplikasi tidak eror/crash
            jenis = str(d.get('jenis_sampah', 'unorganik')).lower()
            
            # Pengaman konversi waktu analisa
            waktu_analisa_raw = d.get('waktu_analisa', 0.5)
            try:
                waktu_analisa_num = float(waktu_analisa_raw)
            except:
                waktu_analisa_num = 0.5
                
            # Pengaman konversi efisiensi
            ...
            data_list.append({
                ...
                'jenis_sampah': jenis,
                'waktu_analisa': f"{waktu_analisa_num:.3f} detik",
                'waktu_analisa_num': waktu_analisa_num, 
                'daur_ulang': 'Bisa' if jenis != 'unorganik' else 'Sulit',
                'efisiensi_daur_ulang': f"{efisiensi_num:.2f}%",
                'efisiensi_num': efisiensi_num, 
                'rekomendasi': d.get('rekomendasi', 'Pilah sampah dengan bijak.')
            })
            
        # Urutkan riwayat berdasarkan waktu terbaru di atas
        if data_list:
            data_list = sorted(data_list, key=lambda x: x['waktu_klasifikasi'], reverse=True)
            
        return data_list
    except Exception as e:
        # Jika eror, kembalikan list kosong agar sistem otomatis beralih ke data simulasi grafis
        return []
...
# --- TAMPILAN HALAMAN LOGIN (ELEGAN & MINIMALIS) ---
if not st.session_state.status_login:
    # Menggunakan pembagi kolom agar kartu login terkunci rapi persis di tengah layar
    col_kiri, col_tengah, col_kanan = st.columns([1, 1.2, 1])
    
    with col_tengah:
        st.markdown("<br><br>", unsafe_allow_html=True) # Jarak dari atas layar
        st.markdown("""
        <div class="login-container" style="margin-top: 0px; padding: 30px; border-radius: 16px; background-color: white; box-shadow: 0 10px 30px rgba(0,0,0,0.08); border-top: 6px solid #2e7d32;">
            <h1 style='text-align: center; color: #1b5e20; font-size: 2.2em; font-weight: 800; margin-bottom: 5px; font-family: sans-serif;'>♻️ Smart Waste</h1>
            <h2 style='text-align: center; color: #2e7d32; font-size: 1.6em; font-weight: 600; margin-top: 0px; margin-bottom: 25px; font-family: sans-serif;'>Classifier</h2>
            <div style='text-align: center; background-color: #e8f5e9; padding: 8px; border-radius: 8px; margin-bottom: 25px;'>
                <span style='color: #1b5e20; font-weight: bold; font-size: 0.95em;'>🔒 LOGIN SISTEM UTAMA</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        # Kolom input ditempatkan di bawah judul kartu secara presisi & berukuran ringkas
        username_input = st.text_input("Username :", placeholder="Ketik username Anda...")
        password_input = st.text_input("Password :", type="password", placeholder="Ketik password Anda...")
        
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("Masuk Aplikasi →", use_container_width=True):
            if cek_login_firebase(username_input, password_input):
...
# --- TAMPILAN APLIKASI UTAMA (JIKA SUDAH LOGIN BERHASIL) ---
    ...
    # 🔄 [REAL-TIME & SINKRON] Tarik data segar dari Firebase setiap kali ganti menu/halaman
    data_cloud = ambil_data_dari_firebase()
    
    # Jika Firebase kosong/gagal koneksi, baru gunakan data simulasi agar grafik tidak crash
    if not data_cloud:
        data_cloud = generate_dummy_data()
        
    # 🕒 [URUTAN PRESISI] Paksa penyortiran ulang berdasarkan tanggal & jam terbaru (Desc)
    try:
        data_cloud = sorted(data_cloud, key=lambda x: str(x['waktu_klasifikasi']), reverse=True)
    except Exception:
        pass
        
    # Simpan ke session state utama agar dibaca oleh semua halaman dashboard & riwayat
    st.session_state.history = data_cloud
    ...
    if page == "Klasifikasi Sampah":
        ...
        # 🔑 1. INEDKS MEMORI: Bikin wadah kuncian session state agar hasil tidak hilang saat rerun
        ...
        if image_data is not None:
            ...
            if st.button("Mulai Analisis AI"):
                if model is None:
                    st.error("File 'model_sampah_ahmad.h5' tidak ditemukan!")
                else:
                    with st.spinner("🤖 AI sedang menganalisis objek..."):
                        ...
                        # 💾 2. SIMPAN KE MEMORI KUNCIAN (Session State)
                        ...
                        # 🌐 Push Firebase Cloud (Menggunakan nama field yang terstandarisasi)
                        if db is not None:
                            try:
                                db.collection('log_sampah').add({
                                    'waktu_klasifikasi': waktu_sekarang,
                        ...
                        # ⚡ UPDATE REAL-TIME LOKAL: Masukkan data baru langsung ke memori riwayat utama
                       # ⚡ UPDATE REAL-TIME LOKAL: Masukkan data baru langsung ke memori riwayat utama
                        data_baru_lokal = {
                            'waktu_klasifikasi': waktu_sekarang,
                            'jenis_sampah': predicted_class,
                            ...
                            # 👇 PERBAIKAN DI SINI: Pastikan ujungnya huruf 'i', bukan 'y'
                            'efisiensi_daur_ulang': f"{skor_efisiensi:.2f}%",
                            'efisiensi_num': float(skor_efisiensi), 
                            'rekomendasi': info['rekomendasi']
                        }
                        
                        # Menyisipkan data baru di urutan paling atas (indeks 0) agar langsung terbaca di halaman lain
                        if 'history' in st.session_state:
                            st.session_state.history.insert(0, data_baru_lokal)
                        
                        st.success("✅ Data berhasil tersimpan ke Firebase Cloud Firestore!")
                        st.balloons()
            
            # 🖥️ 3. TAMPILKAN HASIL DARI KUNCIAN MEMORI (Di luar tombol, anti-hilang)
            if st.session_state.last_prediction is not None:
                res = st.session_state.last_prediction
                
                st.markdown(f"""
                <div class="result-card">
                    <h2 style='text-align: center; color: #1b5e20; margin-bottom: 5px;'>Hasil: {res['predicted_class'].lower()}</h2>
            ...
            # Jika gambar dihapus dari file_uploader/kamera, reset memori kunciannya
            st.session_state.last_prediction = None
    ...
    # --- HALAMAN 2: DASHBOARD SMART WASTE ---
        ...
        if st.session_state.history:
            ...
            pilihan_waktu = st.selectbox("Pilih Jangkauan Data Real-Time:", ["Semua Data", "1 Minggu Terakhir", "1 Bulan Terakhir", "1 Tahun Terakhir"])
            ...
            # Urutkan tabel dashboard dari data paling baru ke lama
            df_filtered = df_filtered.sort_values(by='waktu_klasifikasi', ascending=False)
            ...
            with col2: st.metric("Kategori Dominan", df_filtered['jenis_sampah'].mode()[0].upper() if not df_filtered.empty else "n/a")
            ...
            with filter_col1: kategori_terpilih = st.selectbox("🔍 Filter Kategori:", ["Semua Kategori"] + [c.lower() for c in class_names])
            with filter_col2: wadah_terpilih = st.selectbox("🗑️ Filter Warna Wadah:", ["Semua Tempat Sampah", "Tempat Sampah Hijau", "Tempat Sampah Kuning", "Tempat Sampah Merah/Abu (Residu)"])
            ...
            with row1_col1:
                st.markdown("**Volume Berdasarkan Kategori**")
                if not df_graph.empty:
                    df_counts = df_graph['jenis_sampah'].value_counts().reset_index()
                    df_counts.columns = ['jenis_sampah', 'jumlah_log']
                    
                    # 🧼 Bersihkan data teks dari spasi liar dan pastikan huruf kecil semua
                    df_counts['jenis_sampah'] = df_counts['jenis_sampah'].str.lower().str.strip()
                    
                    # 🔑 SOLUSI PERMANEN: Definisikan kamus warna secara lokal di sini agar anti-NameError
                    color_map_lokal = {
                        'organik': '#2e7d32',
                        'plastik': '#fbc02d',
                        'kertas': '#ff9800',
                    ...
                    # 🎨 Ambil urutan warna yang pas sesuai data kategori yang muncul saat itu
                    warna_sesuai_data = [color_map_lokal.get(kat, '#78909c') for kat in df_counts['jenis_sampah']]
                    
                    # Gambar grafik batang kategori dengan aman
                    fig_bar = px.bar(
                        df_counts, 
                        x='jenis_sampah', 
                        y='jumlah_log', 
                        color='jenis_sampah', 
                        color_discrete_sequence=warna_sesuai_data, 
                        template="simple_white"
                    )
                    st.plotly_chart(fig_bar, use_container_width=True)
                else:
                    st.info("Tidak ada data untuk grafik kategori.")
            
            with row1_col2:
                st.markdown("**Volume Berdasarkan Tempat Sampah**")
                if not df_graph.empty:
                    df_bin_counts = df_graph['warna_tempat_sampah'].value_counts().reset_index()
                    df_bin_counts.columns = ['warna_tempat_sampah', 'jumlah_log']
                    
                    # Kamus warna wadah tempat sampah
                    color_map_bin = {
                    ...
                    # 🎨 Ambil urutan warna wadah secara dinamis
                    warna_bin_sesuai_data = [color_map_bin.get(wadah, '#78909c') for wadah in df_bin_counts['warna_tempat_sampah']]
                    
                    # Gambar grafik batang wadah tempat sampah dengan aman
                    fig_bin = px.bar(
                        df_bin_counts, 
                        x='warna_tempat_sampah', 
                        y='jumlah_log', 
                        color='warna_tempat_sampah', 
                        color_discrete_sequence=warna_bin_sesuai_data, 
                        template="simple_white"
                    )
                    st.plotly_chart(fig_bin, use_container_width=True)
                else:
                    st.info("Tidak ada data untuk grafik wadah.")

            st.subheader("📋 Log Tabel Riwayat Data (Terurut dari yang Terbaru)")
            df_display = df_filtered.drop(columns=['efisiensi_num', 'waktu_analisa_num'], errors='ignore').copy()
            # Memastikan format tanggal rapi di tabel panduan dataframe
    ...
    # --- HALAMAN 4: RIWAYAT KLASIFIKASI ---
        ...
        st.title("📜 Riwayat Hasil Klasifikasi Real-Time")
        st.markdown("Berikut adalah log aktivitas pengklasifikasian sampah berdasarkan urutan waktu terkini:")
        
        if st.session_state.history:
            # Menampilkan hingga 30 data riwayat terbaru dari cloud
            for item in st.session_state.history[:30]: 
                st.info(f"⏳ **[{item['waktu_klasifikasi']}]** &nbsp;|&nbsp; Kategori Sampah: **{item['jenis_sampah'].upper()}** &nbsp;|&nbsp; Efisiensi Sistem: **{item['efisiensi_daur_ulang']}**")
        else:
            st.warning("Belum ada log riwayat klasifikasi yang tersimpan di cloud database.")
