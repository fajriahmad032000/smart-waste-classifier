...
if not st.session_state.status_login:
    ...
    if st.button("Masuk Aplikasi", use_container_width=True):
        if username_input == "admin" or cek_login_firebase(username_input, password_input): # Fallback bypass untuk memudahkan uji coba lokal
...
# --- TAMPILAN APLIKASI UTAMA (JIKA SUDAH LOGIN BERHASIL) ---
    ...
    if page == "Klasifikasi Sampah":
        ...
        if image_data is not None:
            ...
            if st.button("Mulai Analisis AI"):
                if model is None:
                    # Simulasi klasifikasi jika berkas model .h5 tidak ditemukan
                    st.warning("⚠️ Berkas 'model_sampah_ahmad.h5' tidak ditemukan. Sistem mengaktifkan mode simulasi otomatis.")
                    with st.spinner("🤖 AI (Simulasi) sedang menganalisis objek..."):
                        time.sleep(1.5)
                ...
                <div class="result-card">
                    <h2 style='text-align: center; color: #1b5e20; margin-bottom: 5px;'>Hasil: {predicted_class.upper()}</h2>
    ...
    # --- HALAMAN 2: DASHBOARD DASHBOARD SMART WASTE ---
        ...
        if st.session_state.history:
            ...
            if pilihan_waktu == "1 Bulan Terakhir": df_filtered = df[df['waktu_klasifikasi'] >= (sekarang - timedelta(days=30))]
            if pilihan_waktu == "1 Tahun Terakhir": df_filtered = df[df['waktu_klasifikasi'] >= (sekarang - timedelta(days=365))]
            ...
            with row1_col1:
                ...
                if not df_graph.empty:
                    ...
                    # Data Keluar Otomatis Di bawah Grafik
